...
// Covers `metadata.ts`'s `templatesBackendMode() === 'local'` false branch when a bare
// `TEMPLATES_MODEL_NAME` is set without `TEMPLATES_BACKEND=local` — the old `DATABASE_TEMPLATES`
// kill-switch is gone, but the "a stray model name alone doesn't accidentally register anything"
// behavior it used to guard against still matters and still needs its own coverage. See
...
Deno.test({
  ...
  name:
    'defineAdminMetadata() TEMPLATES_BACKEND=remote skips /admin/templates even with TEMPLATES_MODEL_NAME set',
  fn: async () => {
