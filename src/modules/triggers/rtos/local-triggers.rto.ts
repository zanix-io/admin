 ...
 * from `TriggerServiceModelParamsRTO` (`./triggers.rto.ts`) — a proxying consumer's own route also
 * needs a `serviceId` to resolve which service to call, so it needs its own params RTO shape
 * regardless.
 */
